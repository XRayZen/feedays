...
class _ReorderableTreeListViewState
  ...
  Widget build(BuildContext context) {
    ...
    if (res.isEmpty()) {
      ...
      //NOTE:DragAndDropListsはページなら静的な高さは要らないがドロワーメニューでは静的な高さ設定が必要
  ...
  _buildList(List<FeedModel> nodes) {
    ...
    for (var node in nodes) {
      widgets.add(DragAndDropListExpansion(
          ...
          children: _buildTreeChileNodes(node.nodes)));
    }
    return widgets;
  }

  List<DragAndDropItem> _buildTreeChileNodes(List<FeedModel> models) {
    if (models.isEmpty) {
      return [];
    }
    return models.map((e) {
